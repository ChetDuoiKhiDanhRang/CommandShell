...
#mounted point
$mounted = Read-Host "Input WIM mounted path"
$mounted =$mounted.Replace("""","")
if (Test-Path -Path $mounted) {
    ...
    foreach ($d in $dependencies){
         ...
         Add-AppxProvisionedPackage -Path $mounted -SkipLicense -PackagePath $d.fullname
         if ($?) {
    ...
    foreach ($a in $appxs){
         ...
         Add-AppxProvisionedPackage -Path $mounted -SkipLicense -PackagePath $a.fullname
         if ($?) {
         "  >>Success!" >> "$folder\log.txt"
         } else {
         "  >>Fail!" >> "$folder\log.txt"
